    ...
    OutputArray& outputs, ConstantArray& constants, SamplerArray& samplers)
{
    ...
    for (/**/; iter != end; ++iter)
    {
        ...
        if (begin != std::string::npos)
        {
            SamplerTypeMap::iterator iiter = mSamplerTypes.find(tokens[1]);
            if (iiter == mSamplerTypes.end())
            {
                ReportError("Invalid sampler type", &tokens);
                return false;
            }
            samtype = iiter->second;
        }
        else
        {
            VariableTypeMap::iterator iiter = mVariableTypes.find(tokens[1]);
            if (iiter == mVariableTypes.end())
            {
                ReportError("Invalid variable type", &tokens);
                return false;
            }
            vartype = iiter->second;
        }
